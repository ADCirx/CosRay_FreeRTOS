...
static const char *TAG = "MuonDetector";
...
// 生成测试数据
static void generate_test_data(muon_packet_t *packet)
{
    // 设置包头
    ...
    // 计算CRC 校验
    packet->crc = calculate_crc((uint8_t*)packet, sizeof(muon_packet_t) - 2);
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Muon Detector application started");
